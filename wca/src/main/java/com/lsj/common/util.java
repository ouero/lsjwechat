...
public class util {
    ...
    public static void main(String[] args) {
        String s=getSmartName("吴志航");
        System.out.println(s);
    }

    public static String getSmartName(String remarkNames) {
        List<String> list = new ArrayList<>();
        String[] fn = getFirstName();
        String[] rm=remarkNames.split("[\\s()@!！#（）|]");
        String remarkName="";//跳出包含姓名的一段
        for (String s : rm) {
            boolean is=false;
            for (String s1 : fn) {
                if(s.startsWith(s1)){
        ...
        if(remarkName.length()==2||(remarkName.length()==3&&firstName.length()==2)||remarkName.length()-firstName.length()==1){//备注名字单名
            return remarkName;
        }else if(remarkName.length()-firstName.length()>1){//不是单名的
               return  remarkName.substring(firstName.length(),remarkName.length());
        }
        return remarkNames;
    }

    public static String[] getFirstName() {
        String[] fn = {"赵", "钱", "孙", "李", "周", "吴", "郑", "王", "冯", "陈", "褚", "卫",
